 ...
 */
#define LOG_TAG "android.hardware.biometrics.fingerprint@2.3-service.I001D"
#define LOG_VERBOSE "android.hardware.biometrics.fingerprint@2.3-service.I001D"
...
struct SendCmdTask {
    sp<vendor::goodix::hardware::biometrics::fingerprint::V2_1::IGoodixFingerprintDaemon> daemon;
    int32_t cmd;
};

static void* send_cmd_thread(void* arg) {
    auto* task = static_cast<SendCmdTask*>(arg);
    task->daemon->sendCommand(task->cmd, {},
                        [](int32_t, const ::android::hardware::hidl_vec<int8_t>&) {});
    delete task;
    return nullptr;
}

static void sendCommandAsync(
    const sp<vendor::goodix::hardware::biometrics::fingerprint::V2_1::IGoodixFingerprintDaemon>& daemon,
    int32_t cmd) {
    auto* task = new SendCmdTask{daemon, cmd};
    pthread_t thread;
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
    pthread_create(&thread, &attr, send_cmd_thread, task);
...
Return<void> BiometricsFingerprint::onFingerDown(uint32_t, uint32_t, float, float) {
    sendCommandAsync(mGoodixFingerprintDaemon, CMD_FINGER_DOWN);
    sendCommandAsync(mGoodixFingerprintDaemon, CMD_LIGHT_AREA_STABLE);
    android::base::WriteStringToFile(FOD_DISABLE_FALSE, FOD_DISABLE_PATH);
    return Void();
}

Return<void> BiometricsFingerprint::onFingerUp() {
    sendCommandAsync(mGoodixFingerprintDaemon, CMD_FINGER_UP);
